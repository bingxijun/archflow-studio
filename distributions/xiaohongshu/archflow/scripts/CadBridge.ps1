...
function Load-CadLispPlugin {
    ...
    if (-not $LispPath) {
        ...
        $SourcePath = Join-Path $SkillRoot "assets\codex_cad_bridge.lsp.txt"
        if (-not (Test-Path -LiteralPath $SourcePath)) {
            throw "AutoLISP source not found: $SourcePath"
        }
        $GeneratedDir = Join-Path $env:LOCALAPPDATA "ArchFlow\generated"
        New-Item -ItemType Directory -Force -Path $GeneratedDir | Out-Null
        $LispPath = Join-Path $GeneratedDir "codex_cad_bridge.lsp"
        Copy-Item -LiteralPath $SourcePath -Destination $LispPath -Force
    }
...
function Test-CadBridge {
    ...
    $SkillRoot = Split-Path -Parent $PSScriptRoot
    $LispPath = Join-Path $SkillRoot "assets\codex_cad_bridge.lsp.txt"
    ...
    return [pscustomobject]@{
        ...
        lispRuntimePath = (Join-Path $env:LOCALAPPDATA "ArchFlow\generated\codex_cad_bridge.lsp")
