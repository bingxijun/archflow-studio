...
Write-Host "Changes when applied: deploy the ArchFlow-owned SketchUp RBZ with backups and create a local pairing token. MCP host registration remains manual."
...
if (-not $Apply) {
    ...
    Write-Host "`nSetup applied. Register the bundled MCP example with a compatible host, restart SketchUp and the host, open a blank SketchUp model, then call sketchup_bridge_status and sketchup_get_selection."
}
