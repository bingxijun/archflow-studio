...
DEFAULT_RENDER_STYLE_CATALOG = TEMPLATE_DIR / "render_style_catalog.json"
...
def generate_prompts(model: dict[str, Any], output_dir: Path, material_map: dict[str, Any], style_catalog: dict[str, Any]) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    styles = [
        ...
        for item in style_catalog.get("styles", [])
    ]
    manifest = {
        "schema_version": "0.2",
        "project_id": project.get("id"),
        "render_provider": "auto",
        "default_style": style_catalog.get("default_style", "photorealistic-daylight"),
        "available_views": ["current", "front", "right", "top", "axon"],
        "available_styles": styles,
        "geometry_lock": style_catalog.get("geometry_lock", []),
        "negative_prompt": style_catalog.get("base_negative_prompt", ""),
        "creative_assumptions": ["lighting, weather, landscaping, people, loose furniture, and context are unspecified unless supplied separately"],
        "views": [],
    }
    negative = "Do not change the camera, crop, site boundary, footprint, floor count, roof silhouette, wall positions, openings, structural grid, setbacks, finished-floor elevations, or adjacent-site geometry. Do not invent unsupported construction details."
    for key, camera in views.items():
        prompt = (
            ...
            "Keep the visual style neutral until the user selects one; lighting, weather, people, planting, loose furniture, and background atmosphere are creative assumptions. "
            + negative
        )
...
def command_prompts(args: argparse.Namespace) -> int:
    ...
    if report["status"] == "ERROR":
        ...
        return 2
    generate_prompts(model, args.output_dir, read_json(args.material_map), read_json(args.style_catalog))
...
def command_all(args: argparse.Namespace) -> int:
    ...
    generate_prompts(model, args.output_dir / "render_prompts", read_json(args.material_map), read_json(args.style_catalog))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prompts.add_argument("--style-catalog", type=Path, default=DEFAULT_RENDER_STYLE_CATALOG)
    ...
    all_cmd.add_argument("--style-catalog", type=Path, default=DEFAULT_RENDER_STYLE_CATALOG)
