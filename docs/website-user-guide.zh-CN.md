...
ArchFlow Studio 是面向建筑方案与初步设计的可追溯自动化工作流。它把施工或设计要求、敷地 CAD、官方法规证据、建筑语义模型、CAD 草图和 SketchUp 模型脚本连接到同一套 Codex 工作流中。
...
```text
0.1.0-dev.1+codex.20260712095925
```
...
```powershell
Get-FileHash .\ArchFlow-Studio-0.1.0-dev.1+codex.20260712095925-windows.zip -Algorithm SHA256
```
